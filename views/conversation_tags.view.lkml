view: conversation_tags {
 sql_table_name: `support-df-cx-26hwzn7k.df_cx_iva.conversation_tags` ;;
  ...
  dimension: fiscal_quarter {
    ...
    sql:
    CASE
      WHEN EXTRACT(MONTH FROM ${TABLE}.created_at) BETWEEN 4 AND 6 THEN CONCAT('FY', RIGHT(EXTRACT(YEAR FROM ${TABLE}.created_at) + 1, 2), '-Q1')
      WHEN EXTRACT(MONTH FROM ${TABLE}.created_at) BETWEEN 7 AND 9 THEN CONCAT('FY', RIGHT(EXTRACT(YEAR FROM ${TABLE}.created_at) + 1, 2), '-Q2')
      WHEN EXTRACT(MONTH FROM ${TABLE}.created_at) BETWEEN 10 AND 12 THEN CONCAT('FY', RIGHT(EXTRACT(YEAR FROM ${TABLE}.created_at) + 1, 2), '-Q3')
      WHEN EXTRACT(MONTH FROM ${TABLE}.created_at) BETWEEN 1 AND 3 THEN CONCAT('FY', RIGHT(EXTRACT(YEAR FROM ${TABLE}.created_at), 2), '-Q4')
    END ;;
  }
